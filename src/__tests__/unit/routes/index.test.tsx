...
describe("Index Route (Home Page)", () => {
  ...
  it("should initialize keep awake functionality on native platform", async () => {
    const { KeepAwake } = await import("@capacitor-community/keep-awake");
    const { logger } = await import("../../../lib/logger");

    // Create local mocks for platform check
    const isNativePlatformMock = vi.fn().mockReturnValue(true);
    const keepAwakeMock = vi.fn().mockResolvedValue(undefined);
    const allowSleepMock = vi.fn().mockResolvedValue(undefined);

    // Component that mimics the Index behavior with injected mocks
    const KeepAwakeComponent = () => {
      React.useEffect(() => {
        if (!isNativePlatformMock()) return;

        keepAwakeMock().catch((error: Error) => {
        ...
        return () => {
          allowSleepMock().catch((error: Error) => {
    ...
    expect(allowSleepMock).toHaveBeenCalled();

    // Verify the original module mock is also working
    expect(KeepAwake.keepAwake).toBeDefined();
    expect(KeepAwake.allowSleep).toBeDefined();
  });

  it("should skip keep awake on web platform", async () => {
    const { KeepAwake } = await import("@capacitor-community/keep-awake");

    // Create local mocks for platform check - web platform
    const isNativePlatformMock = vi.fn().mockReturnValue(false);
    ...
    // Should not be called on web platform
  ...
  it("should handle keep awake errors gracefully on native platform", async () => {
    const { logger } = await import("../../../lib/logger");

    // Create local mocks for platform check with errors
    const isNativePlatformMock = vi.fn().mockReturnValue(true);
    const keepAwakeError = new Error("KeepAwake not supported");
    const allowSleepError = new Error("AllowSleep not supported");
    const keepAwakeMock = vi.fn().mockRejectedValue(keepAwakeError);
    const allowSleepMock = vi.fn().mockRejectedValue(allowSleepError);
